...
export default {
  user: {
    ...
    async addProduct(req, res, next) {
      try {
        const { product_storing_id, company_id, quantity } = req.body;
...
const changeStore = async ({ cart, updateStore }) => {
  ...
  const isExistProduct =
    isExistOrder &&
    isExistOrder.products.find((_product) => product.id.toString() === _product.id.toString());
  const newProduct = isExistProduct || JSON.parse(JSON.stringify(product));

  const nextProductStoring = await productStoringService.findActiveV2({
    product_id: product.product_id,
    store_id: updateStore.to_store_id,
    populate: 'store'
  });
  const total = (isExistProduct ? newProduct.quantity : 0) + product.quantity;
  if (total > nextProductStoring.stock) newProduct.quantity = nextProductStoring.stock;
  else newProduct.quantity = total;

  const promotionCode = await promotionCodeServiceV2.autoGetV2({
    company_id: product.company_id,
    store_id: updateStore.to_store_id,
    product_id: product.id,
    apply_count: newProduct.quantity,
    promotion_code_id: isExistProduct && isExistProduct.promotion_code_id
  });
  ...
  if (isExistOrder) {
    if (!isExistProduct) isExistOrder.products.push(newProduct);
  } else {
    cart.orders.push({
      ...oldOrder.toObject(),
      _id: undefined,
      ...
      store_address: {
        name: store.name,
        ...nextProductStoring.store.address,
        normalizedAddress: nextProductStoring.store.normalizedAddress
      },
      products: [
        {
          ...newProduct,
          product_id: product.id,
          product_storing_id: nextProductStoring._id,
          store_id: updateStore.to_store_id
        }
      ]
    });
